          ...
          data.map((pack: any) => (
            <div className="m-1 w-full rounded-lg border border-gray-200 bg-white shadow sm:max-w-xs" key={pack.id}>
              <div className="flex flex-col items-center pb-5 pt-4">
                  <Image className="mb-3 h-24 w-24 rounded-full shadow-lg" src={`https://booking.tayyurt-surf.com/storage/${pack.picture}`} alt="Bonnie image" width={100} height={100} />
                <h5 className="text-xl font-medium text-gray-900 text-center">
                  {pack.title}
                  {
                    pack.with_hosting == "0" &&
                    <p className='font-bold text-[10px] text-green-500 underline txt-under-line'>WITHOUTY HOSTING</p>
                  }
                </h5>
